...
function serializePage(doc: any) {
  return {
...
async function findPageByIdentifier(id: string) {
  const filters: Array<Record<string, unknown>> = [{ sourceId: id }];
...
async function ensureUniqueRoute(route: string, sourceId: string) {
...
async function seedPagesFromFrontendStore() {
  ...
  const tutorIdMap = new Map(
    (await TutorModel.find({}, { _id: 1, sourceId: 1 }).lean().exec()).map((item: any) => [
      item.sourceId,
      item._id.toString(),
    ]),
  );
  const reviewIdMap = new Map(
    (await ReviewModel.find({}, { _id: 1, sourceId: 1 }).lean().exec()).map((item: any) => [
      item.sourceId,
      item._id.toString(),
    ]),
  );
    ...
    await PageModel.findOneAndUpdate(
      { sourceId },
      {
        ...
        featuredTutorIds: (page.featuredTutorIds ?? []).map(
          (id: string) => tutorIdMap.get(id) ?? id,
        ),
        featuredReviewIds: (page.featuredReviewIds ?? []).map(
          (id: string) => reviewIdMap.get(id) ?? id,
        ),
        faqItems,
...
function buildPersistedPagePayload(
  payload: PagePayload,
  existing?: any,
) {
  ...
  return {
    ...
    supportPanel:
      payload.supportPanel ?? existing?.supportPanel ?? { title: "", text: "", bullets: [] },
    overview:
      payload.overview ?? existing?.overview ?? { badge: "", title: "", subtitle: "", cards: [] },
    ...
    faqItems:
      payload.faqItems !== undefined ? sanitizeFaqItems(payload.faqItems) : existing?.faqItems ?? [],
